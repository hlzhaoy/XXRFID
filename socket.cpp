...
int initSocket(char* ip, char* port, int timeout)
{
    ...
    if (socketHandle == -1) {
        char tmp[128] = {0};
        sprintf(tmp, "failed to socket: %s", strerror(errno));
        LOG_TICK(tmp);
        return -1;
    }
    ...
    if(tmpPort == 0) {
        char tmp[128] = {0};
        sprintf(tmp, "failed to atoi: %s", port);
        LOG_TICK(tmp);
        return -1;
    }
    ...
    if (ret == -1) {
        if (errno != EINPROGRESS) {
            char tmp[64] = {0};
            sprintf(tmp, "failed to connect: %s", strerror(errno));
            return -1;
        }
        ...
        switch(ret)
        {
            case -1:
                {
                    char tmp[128] = {0};
                    sprintf(tmp, "select error: %s", strerror(errno));
                    LOG_TICK(tmp);
                    return -1;
                }
            case 0:
